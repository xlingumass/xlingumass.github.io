...
/**
 * Calculate the by-word display time.
 */
function calculate_word_time(word, base, boost)
{
    return base + (word.length * boost)
}

function boolToInt(x) { if (x) return 1; else return 0; }

define_ibex_controller({
name: "EPDashedSentence",
...
jqueryWidget: {
    _init: function() {
        ...
        this.wordTime = dget(this.options, "wordTime", this.display == "in place" ? 400 : 300); // Only for speeded acceptability.
        this.characterTimeAdd = dget(this.options, 'characterTimeAdd', 0); // Only for speeded acceptability.
        ...
        if (this.mode == "speeded acceptability") {
            ...
            function wordPauseTimeout() {
                console.log(t.words[t.currentWord]);
                console.log(calculate_word_time(
                                        t.words[t.currentWord],
                                        t.wordTime,
                                        t.characterTimeAdd));

                t.showWord(t.currentWord);
                t.utils.clearTimeout(wordPauseTimeout);
                t.utils.setTimeout(wordTimeout,
                                   calculate_word_time(
                                        t.words[t.currentWord],
                                        t.wordTime,
                                        t.characterTimeAdd));
            ...
            console.log(t.words[t.currentWord]);
            console.log(t.characterTimeAdd);
            this.utils.setTimeout(wordTimeout,
                                  calculate_word_time(
                                      t.words[t.currentWord],
                                      this.wordTime,
                                      this.characterTimeAdd));
        }
